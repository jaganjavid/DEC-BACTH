...
const Header = () => {
  ...
  const LogoutButton = () => {
    ...
    return (
      user && <button className="text-gray-900 bg-white border border-gray-300" onClick={handleLogout}>
      Logout
      </button>
    )
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              {
                navLists.map((item) => (
                  <li key={item.name}>
                      <NavLink to={item.to} className={({isActive}) => isActive ? active : notActive} aria-current="page">{item.name}</NavLink>
                  </li>
                ))
              }
            </ul>
